...
public class FileManager {
	...
	public void createReplicaFiles() {
		int numReplicas = Util.numReplicas;
    ...
    public int distributeReplicastoPeers() throws RemoteException {
		...
		for (BigInteger replica : replicafiles) {
			...
			// add the replica to the successor
			successor.addKey(replica);
			// check if this successor should be the primary
			boolean isPrimary = (counter == index);
			// save the file content on the successor
	...
	public Set<Message> requestActiveNodesForFile(String filename) throws RemoteException {
		...
		for (BigInteger replica : replicafiles) {
			// find its successor
			NodeInterface successor = chordnode.findSuccessor(replica);
			// get the metadata of the replica from the successor
	...
	public NodeInterface findPrimaryOfItem() {
		...
		for (Message message : activeNodesforFile) {
			if (message.isPrimaryServer()) {
				primaryNode = Util.getProcessStub(message.getNodeName(), message.getPort());
				break;
			}
		}
		return primaryNode;
